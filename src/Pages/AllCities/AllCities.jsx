...
export default function AllCities() {
    ...
    const getAllCities = async()=>{
        let arr=[]
        const url = 'https://nomad-list-cities.p.rapidapi.com/nomad-list/europe?size=30&page=1&sort=desc&sort_by=overall_score';
        const options = {
	    method: 'GET',
	    headers: {
        ...
        for(let i=0; i<cities.length; i++){
            const res = await axios.get(`https://api.pexels.com/v1/search/?page=1&per_page=2&query=${cities[i]}`, {
                headers: {
                    Authorization:
            ...
            arr.push({img:res.data?.photos[0]?.src?.original,title:cities[i],id:Math.floor(Math.random()*1000)})
        }
        console.log(arr)
        setCity([...arr])
